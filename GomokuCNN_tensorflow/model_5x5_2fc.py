...
# 1st conv layer
W_conv1 = weight_variable("W_conv1", [5, 5, 1, 512])
b_conv1 = bias_variable("b_conv1", [512])

h_conv1 = tf.nn.relu(conv2d_same(x_board, W_conv1) + b_conv1)
h_conv1_drop = tf.nn.dropout(h_conv1, keep_prob)

# 2nd conv layer
W_conv2 = weight_variable("W_conv2", [3, 3, 512, 256])
...
# 3rd conv layer
W_conv3 = weight_variable("W_conv3", [3, 3, 256, 128])
...
# 1st fc layer
W_fc1 = weight_variable("W_fc1", [15 * 15 * 128, 1024])
...
with tf.Session() as sess:
    ...
    saver.restore(sess, os.path.join(cur_dir, 'tmp_no_pooling/28_28000_model.ckpt'))
    ...
    for epoch in range(32, 33):
        ...
        if epoch == 31:
            ...
            for i in range(28000):
                ...
                sess.run(train_step, feed_dict={x: x_train[batch_start:batch_start + 50], y_answer: y_train[batch_start:batch_start + 50], keep_prob: 0.5})
                if (i + 1) % 28000 == 0:
                   save_path = saver.save(sess, cur_dir + '/tmp_no_pooling/' + str(epoch) + '_' + str(i + 1) + '_model.ckpt')
                   print("Model saved in file: %s" % save_path)
        """
