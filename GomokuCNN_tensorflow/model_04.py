...
y_board_labels = abs(Record.load_output_records(encoding_option='sequence'))
...
y_answer = tf.placeholder(tf.int8, [None, 225])

# 1st conv layer
W_conv1 = weight_variable("W_conv1", [5, 5, 1, 128])
...
# 2nd conv layer
W_conv2 = weight_variable("W_conv2", [3, 3, 128, 128])
...
# 3rd conv layer
W_conv3 = weight_variable("W_conv3", [3, 3, 128, 128])
...
h_conv3_flat = tf.reshape(h_conv3_drop, [-1, 15 * 15 * 128])

# 1st fc layer
W_fc1 = weight_variable("W_fc1", [15 * 15 * 128, 512])
...
# 2nd fc layer
W_fc2 = weight_variable("W_fc2", [512, 225])
b_fc2 = bias_variable("b_fc2", [225])

y_pred = tf.matmul(h_fc1_drop, W_fc2) + b_fc2

saver = tf.train.Saver()

cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_answer, logits=y_pred))

# train & evaluate the model
train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
correct_prediction = tf.equal(tf.argmax(y_pred, 1), tf.argmax(y_answer, 1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))

with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    # saver.restore(sess, os.path.join(cur_dir, 'saved_model_04/6_28000_model.ckpt'))
